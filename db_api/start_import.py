...
class ImportToDB:
    ...
    @staticmethod
    async def update_wallet_instance(session, wallet_instance, token, address, proxy, email):
        has_changed = False
        ...
        if wallet_instance.email_data != email:
            wallet_instance.email_data = email
            has_changed = True
